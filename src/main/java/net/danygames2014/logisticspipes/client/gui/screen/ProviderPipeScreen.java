...
public class ProviderPipeScreen extends LogisticsBaseScreen {
    private final ProviderLogisticPipeBlockEntity pipe;

    public ProviderPipeScreen(PlayerEntity player, ProviderLogisticPipeBlockEntity pipe) {
        super(player, pipe, new ProviderScreenHandler(player, pipe));
        this.pipe = pipe;
        backgroundWidth = 194;
    ...
    protected void drawForeground() {
        ...
        textRenderer.draw(pipe.getFilterInventory().getName(), backgroundWidth / 2 - textRenderer.getWidth(pipe.getFilterInventory().getName()) / 2, 6, 0x404040);
    ...
    protected void buttonClicked(ButtonWidget button) {
        if (button.id == 0) {
            pipe.setFilterExcluded(!pipe.isExcludeFilter());
            if (player.world.isRemote) {
                PacketHelper.send(new ProviderModuleCommandC2SPacket(0, pipe.isExcludeFilter() ? 1 : 0));
            }
        } else if (button.id == 1) {
            pipe.nextExtractionMode();
            if (player.world.isRemote) {
                PacketHelper.send(new ProviderModuleCommandC2SPacket(1, pipe.getExtractionMode().ordinal()));
            }
        }
    ...
    private String getExtractionModeString() {
        return switch (pipe.getExtractionMode()) {
            case Normal -> "Normal";
    ...
    String getContent() {
        return pipe.isExcludeFilter() ? "Exclude" : "Include";
    }
}
