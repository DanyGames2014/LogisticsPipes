...
public class ChassisPipeHud extends BasicGuiHud{
    ...
    public ChassisPipeHud(ChassisLogisticPipeBlockEntity pipe, ChassisModule module, Inventory moduleInventory) {
        this.pipe = pipe;
        this.module = module;
    ...
    public void renderHeadUpDisplay(double distance, boolean day, Minecraft mc) {
        ...
        if(selected != -1) {
            ...
            if(selectedmodule instanceof HUDModuleHandler && ((HUDModuleHandler)selectedmodule).getRenderer() != null) {
                GL11.glTranslatef(11.0F, 5.0F, (float) (-0.00005F * distance));
                ((HUDModuleHandler)selectedmodule).getRenderer().renderContent();
                if(((HUDModuleHandler)selectedmodule).getRenderer().getButtons() != null) {
                    for(HUDButton button:((HUDModuleHandler)selectedmodule).getRenderer().getButtons()) {
                        button.renderAlways();
                ...
                GL11.glTranslatef(0.0F, 0.0F, (float) (-0.00005F * distance));
                mc.textRenderer.draw("Nothing" , -5, -15, 0);
                mc.textRenderer.draw("to" , 9, -5, 0);
                mc.textRenderer.draw("display" , -5, 5, 0);
                GL11.glTranslatef(0.0F, 0.0F, (float) (0.00005F * distance));
            }
        } else {
    ...
    private void moduleClicked(int number) {
        ...
        if(selected != -1) {
            LogisticsModule selectedmodule = module.getSubModule(selected);
            if(selectedmodule instanceof HUDModuleHandler) {
                ((HUDModuleHandler)selectedmodule).startWatching();
            }
        }
    }
    ...
    private void resetSelection() {
        if(selected != -1) {
            LogisticsModule selectedmodule = module.getSubModule(selected);
            if(selectedmodule instanceof HUDModuleHandler) {
                ((HUDModuleHandler)selectedmodule).stopWatching();
            }
        }
        selected = -1;
    }
    ...
    private class ItemButton extends BasicHUDButton {
        ...
        public ItemButton(Inventory inventory, int position, int x, int y, int width, int height) {
        ...
        public void renderButton(boolean hover, boolean clicked) {
            ...
            ItemStack module = inventory.getStack(position);
        ...
        public void renderAlways() {
            if(inventory.getStack(position) == null && shouldDisplayButton(position)) {
                GL11.glPushMatrix();
                GL11.glColor4b((byte)127, (byte)127, (byte)127, (byte)64);
                GL11.glScaled(0.5D, 0.5D, 1.0D);
                Minecraft mc = Minecraft.INSTANCE;
        ...
        @Override
        public boolean shouldRenderButton() {
            boolean result = inventory.getStack(position) != null && shouldDisplayButton(position);
            return result;
        }
